...
/**
 * Created by @adrian on 19/06/14. All rights reserved.
 */
...
public class SecurityConf extends WebSecurityConfigurerAdapter {
	...
	protected void configure(HttpSecurity http) throws Exception {
		...
		http.authorizeRequests()
				...
				// TODO: Not sure if necesary...
				.antMatchers("/site/profile/**").authenticated().antMatchers("/site/vote**").authenticated()
				.antMatchers("/site/public/**").permitAll().antMatchers("/site/**").permitAll()
				.antMatchers("/bazaar/**").permitAll().antMatchers("/auth/**").permitAll()
				.antMatchers("/bazaar/edit**").authenticated().antMatchers("/bazaar/item/delete**").authenticated()
				.antMatchers("/bazaar/item/renew**").authenticated().antMatchers("/bazaar/add**").authenticated()
				.antMatchers("/bazaar/item/fav**").authenticated().antMatchers("/logout").authenticated()
				.antMatchers("/elovendo/messages/public/send").anonymous()
				.antMatchers("/elovendo/messages/**")
				.authenticated()
				.antMatchers("/elovendo/offer/public/send")
				.anonymous()
				.antMatchers("/elovendo/offer/**")
				.authenticated()
				.antMatchers("/elovendo/contact/**")
				.anonymous()
				.and()
				.formLogin()
				...
				.logoutSuccessHandler(new UserLogoutSuccessHandler()).logoutSuccessUrl("/index")
				.deleteCookies("jsessionid", "JSESSIONID").permitAll().and().rememberMe().and().sessionManagement()
				.sessionAuthenticationStrategy(compositeSessionAuthenticationStrategy).and()
