...
public interface ItemRepository extends PagingAndSortingRepository<Item, Long> {
	...
	/**
	 * Finds an item which has featured option enabled, searching for filter in
	 * category or subCategory. Explicitly set a *number* of maximum random
	 * items to retrieve.
	 * 
	 * @param pageable
	 * @param filter
	 *            Category or SubCategory name for query
	...
	// Page<Item> findByTitleMin(@Param("title") String title,
	// @Param("prizeMin") String prizeMin, Pageable pageable);
	//
	// @Query("SELECT i FROM Item i WHERE i.title LIKE %:title% AND prize <= :prizeMax AND endDate > NOW()")
	// Page<Item> findByTitleMax(@Param("title") String title,
	...
	// + "AND endDate > NOW()")
	// Page<Item> findByTitleMinMax(@Param("title") String title,
	// @Param("prizeMin") String prizeMin,
	// @Param("prizeMax") String prizeMax, Pageable pageable);
	 ...
	 */
	 ...
	 Page<Item> findItemsByCategoryName(@Param("categoryName") String
	 ...
	 */
	 ...
	 Page<Item> findItemsByCategoryNameMin(@Param("categoryName") String
	 categoryName,
	 @Param("prizeMin") BigDecimal prizeMin, Pageable pageable);
	
	 @Query("SELECT i FROM Item i WHERE i.subCategory.category.categoryName = :categoryName AND "
	 + "prize <= :prizeMax AND endDate > NOW() ORDER BY startdate")
	 Page<Item> findItemsByCategoryNameMax(@Param("categoryName") String
	 categoryName,
	 @Param("prizeMax") BigDecimal prizeMax, Pageable pageable);
	
	 /**
	 * Find all items from a subCategory filtering by a minimum and maximum
	 prize
	 ...
	 */
	 ...
	 Page<Item> findItemsByCategoryNameMinMax(@Param("categoryName") String
	 categoryName,
	 @Param("prizeMin") BigDecimal prizeMin, @Param("prizeMax") BigDecimal
	 prizeMax, Pageable pageable);
	...
	// Page<Item> findBySubCategoryName(@Param("subCategoryName") String
	// subCategoryName, Pageable pageable);
	//
	// /**
	// * Find all items from a subCategory filtering by a minimum prize
	// * @param subCategoryName SubCategory name
	...
	// Page<Item> findBySubCategoryNameMin(@Param("subCategoryName") String
	// subCategoryName,
	// @Param("prizeMin") BigDecimal prizeMin, Pageable pageable);
	//
	// @Query("SELECT i FROM Item i WHERE i.subCategory.subCategoryName = :subCategoryName AND prize >= :prizeMax "
	// + "AND endDate > NOW() ORDER BY startdate")
	// Page<Item> findBySubCategoryNameMax(@Param("subCategoryName") String
	// subCategoryName,
	// @Param("prizeMax") BigDecimal prizeMax, Pageable pageable);
	//
	// /**
	...
	// Page<Item> findBySubCategoryNameMinMax(@Param("subCategoryName") String
	// subCategoryName,
	// @Param("prizeMin") BigDecimal prizeMin, @Param("prizeMax") BigDecimal
	// prizeMax, Pageable pageable);
	...
	@Query("SELECT new Item(i, (6371*acos(cos(radians(:lat))*cosRadLat*cos(radLng-radians(:lng))+sin(radians(:lat))*sinRadLat))) "
			...
			+ "AND (:pMin <= 0.0 OR normalizedPrize >= :pMin) AND (:pMax <= 0.0 OR normalizedPrize <= :pMax) AND "
	...
	Page<Item> findByParams(@Param("title") String title, @Param("name") String name, @Param("lat") double latitude,
			@Param("lng") double longitude, @Param("dis") double distance, @Param("pMin") double prizemin,
	...
	@Query("SELECT new Item(i, (6371*acos(cos(radians(:lat))*cosRadLat*cos(radLng-radians(:lng))+sin(radians(:lat))*sinRadLat))) "
			...
			+ "AND (:dis >= 5000.0 OR "
			+ "(6371*acos(cos(radians(:lat))*cosRadLat*cos(radLng-radians(:lng))+sin(radians(:lat))*sinRadLat)) < :dis) ORDER BY startDate DESC")
	Page<Item> findByParams(@Param("title") String title, @Param("cat") long cat,
			@Param("isCategory") boolean isCategory, @Param("lat") double latitude, @Param("lng") double longitude,
			@Param("dis") double distance, @Param("pMin") double prizemin, @Param("pMax") double prizemax,
			Pageable pageable);
}